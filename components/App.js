...

            {
              ...
              :
                <div>
...

                    {
                      ...
                      :
                        <div>
                          ...
                          <label htmlFor="increase-impact">Offset these fees by adding {accounting.formatMoney(this.calcImpact())} to my donation</label>
                        </div>
                    }
                  </div>
                  ...
                  <button onClick={this.previousPage.bind(this)}>Back</button>
                  <button id="donate-button" onClick={() => submit(this.state,this.errorCallback.bind(this))}>{this.state.type=="credit" ? "Enter Payment Info" : `Donate $${this.state.increaseImpact ? accounting.formatMoney(this.state.amount+this.calcImpact()) : accounting.formatMoney(this.state.amount)}${this.state.recurring ? ` per ${this.state.frequency.substring(0,this.state.frequency.length-2)}` : ""}`}</button>
                </div>
            }
          </div>
        </div>
